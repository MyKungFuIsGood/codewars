...
function UriBuilder(input) {
    qmark = input.indexOf('?');
    if( qmark > 0 ) {
        this.base = input.slice(0,qmark);
        this.params = {};
        
        var params = input.slice(qmark+1).split('&');
        for( i in params ) {
            pair = params[i].split('=');
    ...
    this.build = function() {
        ...
        if( result === '' ) {
            ...
            return this.base + '?' + result.slice(0,result.length-1).replace(/\s/, '%20');
        }
    }
}